...
class LaporanAdapter(private val onClick: (LaporanModel) -> Unit) : RecyclerView.Adapter<LaporanAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.apply {
            // PERBAIKAN: Gunakan tipe_laporan sesuai model baru
            tvNamaTugas.text = "${item.nama_barang} (${item.tipe_laporan})"

            // PERBAIKAN: Gunakan status (tanpa underscore)
            tvStatus.text = "Status: ${item.status.replace("_", " ")}"

            tvPIC.text = "Villa: ${item.villa_nama}"
